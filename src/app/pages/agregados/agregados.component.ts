...
ngOnInit() {
  ...
  this.as.atualizarbucket("OYGBZIs2mVu43LuFptr1", this.sumcompr);
    ...
    this.fs.collection('sub_agregados', (ref)=> ref.where("cod", "in", [37,38])).valueChanges({idField: 'id'}).subscribe(value =>  {
      this.sub = value;
  ...
  this.as.atualizarbucket("ATWm8m90ckEWSgGVgih7", this.sum);
    ...
    this.fs.collection('sub_agregados', (ref)=> ref.where("cod", "in", [41,39])).valueChanges({idField: 'id'}).subscribe(value =>  {
      this.sub = value;
